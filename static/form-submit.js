...
var formSubmit = new (function () {
  ...
  self.unload = function () {
    var el,
      x,
      counters = document.querySelectorAll('data-form-submit-count')
  ...
  self.validation = new (function () {
    ...
    vself.isTimestamp = function (value, format, generalSeparators) {
      return getRegexFromString(format || 'mm/dd/yyyy HH24:MM:SS.MS', generalSeparators).test(value)
    }
    ...
    vself.formatDigits = function (value, format) {
      ...
      return format
        .replace(/\d/g, function (item, offset) {
          if (digits.length) {
            return digits.shift()
    ...
    vself.formatTimestamp = function (value, format) {
      ...
      if (!value) {
        // Can't format an empty string
        return ''
      }
      // Expected format
      tokenFormat = format.replace(/mm|dd|HH|MM|SS/g, function (item) {
        // Change double letters to single letters in regex
      ...
      if (replacements['H']) {
        ...
        if ((groups = value.match(/(a|p)/i))) {
          // AM/PM affects 24-hour clock only
      ...
      return (
        format
          .replace(/mm|m|dd|d|yyyy|yy|HH24|H24|HH|H|MS|MM|M|SS|S/g, function (item, offset) {
      ...
      ) // Return original value if result is empty string
    }
    ...
    var getRegexFromString = function (str, generalSeparators, optionalValues) {
      ...
      return new RegExp(
        '^\\D*' +
          str.replace(/mm|m|dd|d|yyyy|yy|HH24|H24|HH|H|MS|MM|M|SS|S|.+?/g, function (item) {
  ...
  var removeFormValidation = function (form) {
    ...
    if (form) {
      ...
      for (var x in validationCallbacks) {
        ...
        if (el && el.form == form) {
          return
        } // This form still has a field being validated
      }
  ...
  var documentLoaded = function (ev) {
    ...
    for (x = 0; (el = fields[x]); x++) {
      if ((attrValue = el.getAttribute('data-form-submit-required'))) {
        ...
        switch (attrValue) {
          case 'true':
            assisterValidate(el, function (value) {
              return !!value
            })
            break
          case 'radio':
          ...
          case 'url':
            ...
            break
          case 'url-http':
            assisterValidateAndFormat(el, self.validation.isURLHTTP, self.validation.formatURLHTTP)
            break
          case 'url-path':
    ...
    for (x = 0; (el = fields[x]); x++) {
      if ((attrValue = el.getAttribute('data-form-submit-optional'))) {
        ...
        switch (attrValue) {
          case 'digits':
            assisterOptionalFormat(el, self.validation.isDigits, self.validation.formatDigits)
            break
          case 'number':
            assisterOptionalFormat(el, self.validation.isNumber, self.validation.formatNumber)
            break
          case 'currency':
            assisterOptionalFormat(el, self.validation.isCurrency, self.validation.formatCurrency)
            break
          case 'phone':
            assisterOptionalFormat(el, self.validation.isPhone, self.validation.formatPhone)
            break
          case 'zip':
            assisterOptionalFormat(el, self.validation.isZip, self.validation.formatZip)
            break
          case 'zip+4':
            assisterOptionalFormat(el, self.validation.isZipPlus4, self.validation.formatZipPlus4)
            break
          case 'zip-full':
            assisterOptionalFormat(el, self.validation.isZipFull, self.validation.formatZipFull)
            break
          case 'email':
          ...
          case 'url':
            ...
            break
          case 'url-http':
            assisterOptionalFormat(el, self.validation.isURLHTTP, self.validation.formatURLHTTP)
            break
          case 'url-path':
            assisterOptionalFormat(el, self.validation.isURLPath, self.validation.formatURLPath)
            break
          case 'hostname':
            assisterOptionalFormat(el, self.validation.isHostname, self.validation.formatHostname)
            break
          case 'domain':
            assisterOptionalFormat(el, self.validation.isDomain, self.validation.formatDomain)
            break
          case 'ip-address':
            assisterOptionalFormat(el, self.validation.isIPAddress, self.validation.formatIPAddress)
            break
          case 'timestamp':
            assisterOptionalFormat(el, self.validation.isTimestamp, self.validation.formatTimestamp)
            break
          case 'date-mmddyyyy':
