...
aws s3 sync . s3://dbowland-ui-source/dbowland-ui --exclude "*.html" --exclude "*.json" --exclude "*.xml" \
  --metadata-directive REPLACE --cache-control "public, max-age=31536000, immutable" --acl public-read
# Do not cache
aws s3 sync . s3://dbowland-ui-source/dbowland-ui --include "*.html" --include "*.json" --include "*.xml" \
  --metadata-directive REPLACE --cache-control "public, no-cache" --acl public-read
# Cleanup unused files
aws s3 sync . s3://dbowland-ui-source/dbowland-ui --delete
